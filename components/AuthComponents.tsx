...
export function AuthSwitch({ isLogin, onToggle }: AuthSwitchProps) {
  return (
    <div className="relative flex items-center bg-[#3a3e45] rounded-full w-[270px] h-[44px] mx-auto">
      <button
...
export const CheckboxAuth = ({
  ...
  return (
    <CheckboxPrimitive.Root
      data-slot="checkbox"
      className={cn(
        "peer border-white data-[state=checked]:border-white data-[state=checked]:bg-transparent size-5 shrink-0 rounded-[4px] border shadow-xs transition-colors outline-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/50 disabled:cursor-not-allowed disabled:opacity-50",
        className
      )}
      {...props}
  ...
  return (
    <div className="w-6 h-6 bg-white rounded-full flex items-center justify-center shadow-sm hover:bg-gray-100 transition-colors">
      <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M18 6L6 18" stroke="#374151" strokeWidth="3" strokeLinecap="round"/>
        <path d="M6 6L18 18" stroke="#374151" strokeWidth="3" strokeLinecap="round"/>
      </svg>
    </div>
  )
}
